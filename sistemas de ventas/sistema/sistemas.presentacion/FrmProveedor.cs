...
namespace sistemas.presentacion
{
    public partial class FrmProveedor : Form
    {
        ...
        private void BtnInsertar_Click(object sender, EventArgs e)
        {
            ...
            {
                string Rpta = "";
                if (TxtNombre.Text == string.Empty || TxtNumDocumento.Text == string.Empty || CboTipoDocumento.Text == string.Empty)
                {
                    ...
                    ErrorIcono.SetError(TxtNumDocumento, "Debes ingresar un numero de documento");
                }
                else
                {
        ...
        private void BtnActualizar_Click(object sender, EventArgs e)
        {
            ...
            {
                string Rpta = "";
                if (TxtId.Text==string.Empty || TxtNombre.Text == string.Empty || TxtNumDocumento.Text == string.Empty || CboTipoDocumento.Text == string.Empty)
                {
                    this.MensajeError("Falta ingresar algunos datos.");
                    ErrorIcono.SetError(CboTipoDocumento, "Seleccione un Tipo de documento");
                    ErrorIcono.SetError(TxtNombre, "Ingrese un nombre.");
                ...
                {
                    Rpta = NPersona.Actualizar(Convert.ToInt32(TxtId.Text),"Proveedor", TxtNombre.Text.Trim(), CboTipoDocumento.Text, this.NumDocumentoAnt,TxtNumDocumento.Text.Trim(), TxtDireccion.Text.Trim(), TxtTelefono.Text.Trim(), TxtEmail.Text.Trim());
                    if (Rpta.Equals("OK"))
                    {
                ...
                if (Opcion == DialogResult.OK)
                {
                    ...
                    foreach (DataGridViewRow row in DgvListado.Rows)
                    {
                        if (Convert.ToBoolean(row.Cells[0].Value))
                        {
                            ...
                            if (Rpta.Equals("OK"))
                            {
                                this.MensajeOk("Se elimino correctamente " + Convert.ToString(row.Cells[3].Value));
                            }
                            else
                            {
                                this.MensajeError(Rpta);
