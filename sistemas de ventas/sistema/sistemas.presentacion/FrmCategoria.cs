...
namespace sistemas.presentacion
{
    public partial class FrmCategoria : Form
    {
        ...
        private void Formato()
        {
            ...
            DgvListado.Columns[3].HeaderText = "Descripciòn";
            DgvListado.Columns[3].Width = 650;
                ...
                if (Opcion==DialogResult.OK)
                {
                    ...
                    foreach(DataGridViewRow row in DgvListado.Rows)
                    {
                        if (Convert.ToBoolean(row.Cells[0].Value))
                        {
                            ...
                            if (Rpta.Equals("OK"))
                            {
                                this.MensajeOk("Se elimino correctamente "+ Convert.ToString(row.Cells[2].Value));
                            }
                            else
                            {
                                this.MensajeError(Rpta);
                ...
                if (Opcion == DialogResult.OK)
                {
                    ...
                    foreach (DataGridViewRow row in DgvListado.Rows)
                    {
                        if (Convert.ToBoolean(row.Cells[0].Value))
                        {
                            ...
                            if (Rpta.Equals("OK"))
                            {
                                this.MensajeOk("Se activo correctamente " + Convert.ToString(row.Cells[2].Value));
                            }
                            else
                            {
                                this.MensajeError(Rpta);
                ...
                if (Opcion == DialogResult.OK)
                {
                    ...
                    foreach (DataGridViewRow row in DgvListado.Rows)
                    {
                        if (Convert.ToBoolean(row.Cells[0].Value))
                        {
                            ...
                            if (Rpta.Equals("OK"))
                            {
                                this.MensajeOk("Se desactivo correctamente " + Convert.ToString(row.Cells[2].Value));
                            }
                            else
                            {
                                this.MensajeError(Rpta);
